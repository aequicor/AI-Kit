Proactively look up documentation when working with an unfamiliar library, new instruction set, or any topic where knowledge is missing or uncertain. Searches the local index first; if nothing is found, researches the internet and indexes the result for future use.
...
Always start with the knowledge backend's `find_or_create` operation:
...
| Tool | Source | Purpose |
|------|--------|---------|
| `find_or_create` | knowledge backend | Primary search + auto-create via sampling |
| `add_document` | knowledge backend | Persist manually researched documents |
| `WebSearch` | builtin | Fallback research when sampling is unavailable |
| `WebFetch` | builtin | Read source pages during fallback research |

> If a knowledge backend is unavailable — skip lookup, go directly to webfetch using a canonical URL.

## Error Handling

| Situation | Action |
|-----------|--------|
| `find_or_create` returns no results and sampling unavailable | Follow fallback: WebSearch → WebFetch → add_document. |
...
| `add_document` fails or indexing unavailable | Save the researched content as a local file: `vault/specs/guidelines/[module]/lookup-[topic].md`. Report path. |
