...
| **Minor** (`X.Y+1.0`) | Breaking change to the bundled template tree: a profile, target adapter, dialect, prompt body, command, or skill is **added or removed** (existing manifests referencing old names break, or new names become available). Adding a new manifest field consumed by the renderer (e.g. `policies.optional_skills`) also lands here. |
